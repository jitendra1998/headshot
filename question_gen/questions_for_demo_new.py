...
class GameTableMapping(object):
    def __init__(self):
        ...
        with open('football.json') as json_data:
            self.football = json.load(json_data)

        with open('tennis.json') as json_data:
            self.tennis = (json.load(json_data))

        with open('social_sciences.json') as json_data:
            self.social_science = (json.load(json_data))

        with open('sciences.json') as json_data:
            self.science = (json.load(json_data))

    def random_generator(self, theme):

        if theme.lower() == 'tennis':
            tennis_num = list(self.tennis.keys())
            return int(random.choice(tennis_num))

        elif theme.lower() == 'social_science':
            social_science = list(self.social_science.keys())
            return int(random.choice(social_science))

        elif theme.lower() == 'science':
            science = list(self.science.keys())
            return int(random.choice(science))

        else:
            football_num = list(self.football.keys())
            return int(random.choice(football_num))
...
class QuestionGen(tornado.web.RequestHandler):
    ...
    def post(self):
        ...
        with open('tennis.json') as json_data:
            return_dict.update(json.load(json_data))
